...
while ~stop
    ...
    indicator_d = inf;
    iter_d = 0;
    while indicator_d >= indicator_d_max && iter_d < iter_d_max

        iter_d = iter_d+1;

        % update surrogate and ymin
        [d_outer,idx] = unique(d_outer,'rows');
        f_outer = f_outer(idx,:);
    ...
    for i = 1:n_dmin
        ...
        for obj = 1:n_obj
            ...
            indicator_u = inf;
            iter_u = 0;
            while indicator_u >= indicator_u_max && iter_u < iter_u_max
                iter_u = iter_u + 1;

                % update surrogate and ymin
                [u_inner,idx] = unique(u_inner,'rows');
                f_inner = f_inner(idx,:);
    ...
    nfeval_val = size_u_record*size(dmin,1);
...
% Everything from here i am making up and will need to cross-check... besides i think fevals can be saved by remembering u from outer loop
[f_val, u_val_record, nfeval_aux] = u_validation(problem_max_u, d_record, u_record, lsflag_validation, 1:n_obj);
nfeval = nfeval + nfeval_aux;
% nfeval_validation = nfeval_aux %just to see if they are many 
% nfeval_val

%% Select non-dominated solutions and define outputs
sel = dominance(f_val,0) == 0;
