...
for obj = par_objfun.objectives
    for idx_u = 1:size(par_objfun.u_record{obj},1)
        [y_aux,mse_aux] = par_objfun.surrogate.predictor([d par_objfun.u_record{obj}(idx_u,:)], par_objfun.surrogate.model);
        if (idx_u == 1 || par_objfun.sign_inner*y_aux(1,obj) > par_objfun.sign_inner*y(1,obj))
            y(1,obj) = y_aux(1,obj);
...
masked = -par_objfun.surrogate.indicator(y,mse,par_objfun.ymin);
