function [xsamp,fsamp,maxC,nfeval]=social(x,f,memories,x_DE,cid,nfeval,params)
...
if arch_vs_pop&&n_mem>2                                                     % if we are taking samples from the archive and the archive contains at least 3 elements
...
switch params.DE_strategy
    ...
    case 'DE/current-to-rand/1'
        ...
        if (arch_vs_pop) && (n_mem>2)                                       % if we are sampling from the archive and it contains at least 3 elements
        
            xsamp=xsamp+K*(memories(P(3),1:lx)-xsamp)+K*params.F*(memories(P(1),1:lx)-memories(P(2),1:lx)); % xsamp is evolved with differentiation vector given by agents P(1) P(2) and P(3) FROM THE ARCHIVE
            ...
            if size(id_pop,1)==1
                ...
                if size(id_pop,1)==2
                    ...
                    xsamp=xsamp+K*(x_DE(id_pop(3))-xsamp)+K*params.F*(x_DE(id_pop(1))-x_DE(id_pop(2)));
