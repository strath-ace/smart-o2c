function [xtrial,vtrial,ftrial,maxC,nfeval,discarded,rho,patdirs,MBH_positions]=explore_smooth_scal(memories,x,v,f,cid,nfeval,lambda,act_subpr,id_pop_act_subpr,z,zstar,rho,patdirs,pigr,MBH_positions,params)
%
...
for i=1:n_a                                                                 % for all agents which will perform LOCAL ACTIONS
    ...
    if norm(v(i,:))~=0 && ~isnan(norm(v(i,:)))                                              % if velocity of that agent is not null (velocity is supplied as a parameter, so has been computed in previous steps). THIS IS ALWAYS FALSE DURING THE FIRST CALL OF EXPLORE
        
        alph=1;                                                 % create another random number alpha between 0 and 1
    ...
    if impr(i)==0 && strcmp(params.pat_search_strategy,'standard')
    ...
    if impr(i)==0 && strcmp(params.pat_search_strategy,'tracking')
    ...
    if impr(i)==0 && strcmp(params.pat_search_strategy,'random')
    ...
    %% GRADIENT BASED MINIMISATION OF SMOOTH CHEBYSHEV SCALARISATION
