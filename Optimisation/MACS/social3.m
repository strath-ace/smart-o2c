function [xsamp,fsamp,maxC,nfeval]=social3(x,f,memories,x_DE,cid,nfeval,energy,ener2,params)
...
if arch_vs_pop&&&n_mem>4                                                     % if we are taking samples from the archive and the archive contains at least 3 elements

    tmp_dist=(sum(((memories(:,1:lx)-repmat(xsamp,n_mem,1))./repmat(params.vub-params.vlb,n_mem,1)).^2,2)).^0.5; % fast one-liner to compute distances from all agents in memory to xsample
    ...
    tmp_dist=(sum(((x_DE-repmat(xsamp,pop_DE,1))./repmat(params.vub-params.vlb,pop_DE,1)).^2,2)).^0.5; % fast one-liner to compute distances from all agents in population to xsample
...
switch params.DE_strategy
    ...
    case 'DE/rand/1'
        ...
        if (arch_vs_pop) && (n_mem>2)                                       % if we are taking samples from archives, and there are at least 3 samples in it (remember that we have excluded x itself!)
            ...
            xsamp=xsamp.*(1-e)+e.*(memories(P(3),1:lx)+params.F*(memories(P(1),1:lx)-memories(P(2),1:lx))); % xsamp is evolved with differentiation vector given by agents P(1), P(2) and P(3) FROM THE ARCHIVE
            ...
            if size(id_pop,1)<2
                ...
                if size(id_pop,1)==2
                    ...
                    xsamp=xsamp.*(1-e)+e.*(params.F*(x_DE(id_pop(1),:)-x_DE(id_pop(2),:)));  % xsamp is evolved with differentiation vector given by agents id_pop(1), id_pop(2) and id_pop(3) FROM THE POPULATION
                    
                else
                    
                    xsamp=xsamp.*(1-e)+e.*(x_DE(id_pop(3),:)+params.F*(x_DE(id_pop(1),:)-x_DE(id_pop(2),:)));  % xsamp is evolved with differentiation vector given by agents id_pop(1), id_pop(2) and id_pop(3) FROM THE POPULATION
    ...
    case 'DE/current-to-rand/1'
        ...
        K2 = K;%rand;
        if (arch_vs_pop) && (n_mem>4)                                       % if we are sampling from the archive and it contains at least 3 elements
        
            xsamp=xsamp+K*(memories(P(3),1:lx)-xsamp)+K2*params.F*(memories(P(1),1:lx)-memories(P(2),1:lx)); % xsamp is evolved with differentiation vector given by agents P(1) P(2) and P(3) FROM THE ARCHIVE
            ...
            if size(id_pop,1)<2
                ...
                if size(id_pop,1)==2
                    ...
                    xsamp=xsamp+K*params.F*(x_DE(id_pop(1))-x_DE(id_pop(2)));
                    
                else
                    
                    xsamp=xsamp+K*(x_DE(id_pop(3))-xsamp)+K2*params.F*(x_DE(id_pop(1))-x_DE(id_pop(2)));
...
mask = (xsamp<params.vlb)+(xsamp>params.vub);                                             % mask vector, contains 1 where xsamp is out of bounds
xnew = mask.*rand(1,lx).*(params.vub-params.vlb)+params.vlb;                                     % xnew, vector with random components where xsamp is out of bounds AND ZERO ELSEWHERE
xsamp = xsamp.*~mask+xnew.*mask;                                            % in bounds xsamp
