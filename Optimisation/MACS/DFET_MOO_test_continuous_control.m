...
a=1e-2;
f = @(x,u,t) [x(2); x(4)^2/x(1)-1/x(1)^2+a*cos(x(5)); x(4)/x(1); -x(2)*x(4)/x(1)+a*sin(x(5)); u(1)];
dfx = @(x,u,t) [0 1 0 0 0; -x(4)^2/x(1)^2+2/(x(1)^3) 0 0 2*x(4)/x(1) -a*sin(x(5)); -x(4)/(x(1)^2) 0 0 1/x(1) 0; x(2)*x(4)/(x(1)^2) -x(4)/x(1) 0 -x(2)/x(1) a*cos(x(5)); 0 0 0 0 0];
dfu = @(x,u,t) [0; 0; 0; 0; 1];
dft = @(x,u,t) [0; 0; 0; 0; 0];

t_0 = 0;

x_0 = [1.1 0 0 1/(1.1)^0.5 pi/2];

imposed_final_states = [0 0 0 0 0];   % mask vector with the variables on which a final condition is imposed
x_f = [0 0 0 0 0];                % vector of final conditions (size???)
...
state_bounds = [0 inf;-inf inf; -inf inf;-inf inf; -pi pi];
...
opt.vars_to_opt = logical(~isinf(vlb)-(vlb==0)-(vlb==-pi));
opt.vars_to_opt(2) = 1;
...
opt.oc.state_vars = logical(isinf(vlb)+(vlb==0))+(vlb==-pi);
opt.oc.state_vars(2) = 0;
...
for i=1:1
    ...
    mem(i).memory=macs7v16OC(@max_ener_MACS_MOO2,[],vlb,vub,opt,[],[],vlb,vub,f,structure,x_0,x_f,dfx,dfu,dft,fminconoptions);    
