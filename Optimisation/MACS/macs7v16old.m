function [memory,nfeval,ener]=macs7v16old(func,memory,vlb,vub,options,filename,fileload,varargin)
...
%     if size(memory,1) == options.max_arch
%        keyboard 
%     end
       
    %% INDIVIDUALISTIC MOVES, SELECTION AND ARCHIVING
    
    [xtrial,vtrial,ftrial,maxC,nfeval,discarded,rho]=exploreold(expl_agents,memory,x,v,f,cid,...
        id_pop_act_subpr,act_subpr,lambda,z,nfeval,lx,mfit,options.F,options.CR,rho,vlb,vub,options.coord_ratio,options.contr_ratio,1e-3,options.rhoini,func,options.cp,pigr,options.MBHflag,options.cpat,ncon,varargin);

    %[xtrial,vtrial,ftrial,maxC,nfeval,discarded,rho,impr]=explore(expl_agents,memory,x,v,f,cid,nfeval,lambda,act_subpr,id_pop_act_subpr,z,rho,explore_params);
...
%                v(i,:) = 0*v(i,:);
    ...
    ztmp=ftrial;
    for i=1:size(expl_agents,2)
        ztmp=[ztmp;discarded(i).f];
    end
     
    z=min([z;ztmp],[],1);
    
%    z=min([z;ftrial;discarded.f;memory(:,lx+1:lx+mfit)],[],1);            % update the min of each function (i.e, the columnwise min).
    
    ffeas = [];
    xfeas = [];
    cfeas = [];
    
    for i = 1:length(expl_agents)
       ffeas = [ffeas; discarded(i).f];
       xfeas = [xfeas; discarded(i).x];
       cfeas = [cfeas; discarded(i).c];
    end
    ...
    if ~isempty(csoc)                                                       % if we have actually performed social moves
        ...
        for i = 1:n_social %problems
    
            sel = 0;
            val = g_fun(f(id_pop_act_subpr(i),:),lambda(act_subpr(i),:),z);
    
            for j = 1:size(avail,1) %new points
    
                if (avail(j,end)<=0)                                                    % if this social action is in feasible region
    
                    if (g_fun(avail(j,lx+1:lx+mfit),lambda(act_subpr(i),:),z)<val)||all(avail(j,lx+1:lx+mfit)<=f(id_pop_act_subpr(i),:)) % if sampled position for agent solving i-th subproblem is good (i.e it's gfun is better than the gfun for the old agent)
    
                        sel = j;
                        val = g_fun(avail(j,lx+1:lx+mfit),lambda(act_subpr(i),:),z);
            ...
            if sel>0
                ...
                %v(id_pop_act_subpr(i),:)=0*v(id_pop_act_subpr(i),:);
...
%         end
%         
%         if size(avail_memory,1)>(options.popsize-mfit)  % more memories than remaining agents, free choice
%             
