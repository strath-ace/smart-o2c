function [val,x_sol] = fastest_climb_MACS_MOO_penalise(x_in,lb,ub,f,structure,x_0,x_f,dfx,dfu,dft,fminconoptions)

[x_sol,~,exitflag,output] = fmincon(@(x) 1,x_in(2:end),[],[],[],[],lb(2:end),ub(2:end),@(x) dynamics2(f,structure,x,x_0,x_f,x_in(1),dfx,dfu),fminconoptions);
...
val = val + output.constrviolation*10^6*(exitflag<1);
