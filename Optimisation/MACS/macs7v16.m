...
if isfield(options,'pat_search_strategy')
    if ~(strcmp(options.pat_search_strategy,'standard') || strcmp(options.pat_search_strategy,'tracking') || strcmp(options.pat_search_strategy,'none'))
        error('Unrecognized pat_search_strategy: must be either standard or tracking')
    end
else
...
while (nfeval<options.maxnfeval)
    ...
    [xtrial,vtrial,ftrial,maxC,nfeval,discarded,rho,patdirs]=explore(memory,x,v,f,cid,nfeval,lambda,act_subpr,id_pop_act_subpr,z,rho,patdirs,pigr,explore_params);
