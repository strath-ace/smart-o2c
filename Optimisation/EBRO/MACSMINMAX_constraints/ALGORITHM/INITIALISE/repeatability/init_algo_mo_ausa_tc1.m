...
% parameters
    if isfield(minmax_problem,'maxnfeval')
        par_minmax.maxnfeval = minmax_problem.maxnfeval/4;% TOTAL function evaluation limit
    else
...
if (minmax_problem.n_obj == 1)
    ...
    %parameters{
        par_macs = struct;
        par_macs.maxnfeval = 50*minmax_problem.dim_d;      % Max function evaluations
        ...
        par_macs.max_arch_out = 4;                          % Output archive size
    ...
    %parameters{
        par_macs = struct;
        par_macs.maxnfeval = 600*minmax_problem.dim_d;      % Max function evaluations
        par_macs.popsize = 10 + minmax_problem.dim_d;       % Population size
        par_macs.rhoini = 1;                                % Initial local hypercube size
        par_macs.F = 0.9;                                   % F
        par_macs.CR = 0.9;                                  % CR
        par_macs.p_social = 0.2;                            % Ratio between elite and total population
        par_macs.max_arch = 100;                            % Inner archive size
        par_macs.max_arch_out = 6;                          % Output archive size
        par_macs.coord_ratio = 1;                           % Quota of coordinates examined for each set of individualistic actions
        par_macs.contr_ratio = 0.5;                         % Contraction ratio
        par_macs.draw_flag = -1;                             % Print itarations status  
        par_macs.cp = 0;                                    % constraint flag
...
obj = 1;
    ...
    par_mpaidea = struct;
        par_mpaidea.nFeValMax = 100*minmax_problem.dim_u;           % number of function evaluations for IDEA
