function [memories,nfeval,flag_restart]=idea2pop(func,cfunc,y,vlb0,vub0,options,extraflag,varargin)
%%
flag_restart=0;
lx=length(vlb0);
...
%% COM SECTION
    ...
    fitness(1)= func(y(1,:),memories,[],[],varargin{:});
    
    nfeval  = nfeval + 1;
for i=2:npop                        % check the remaining members
    [fitness(i),fitness(1:i-1)] = func(y(i,:),memories,y(1:i-1,:),fitness(1:i-1),varargin{:});
    nfeval  = nfeval + 1;
end
...
while nostop
    ...
    for i=1:npop
        ...
        if norm(x-y(i,:))>0
            ...
            [tempval, fitness] = func(x,memories,y,fitness,varargin{:});% check cost of competitor
            nfeval  = nfeval + 1;
        else
            tempval=fitness(i);
        end
        ...
        if impr==100
            ...
            while j<1&&impr==0
                ...
                % evaluates step
                if norm(x-y(i,:))>0
                    tempval = func(x,memories,varargin{:});
                    nfeval  = nfeval + 1;
                else
                    tempval=fitness(i);
                end
    ...
    if maxd<convtol*MAXmaxd
        ...
        [xgrad,fvalgrad,exitflag,output]=fmincon(func,arch(idbestf,1:lx),[],[],[],[],vlb0,vub0,cfunc,foptionsNLP,memories,y,fitness,varargin{:});
        ...
        if local_restart<max_local_restart
            ...
            fitness(1)=func(y(1,:),memories,[],[],varargin{:});
            C=cfunc(y(1,:),memories,[],[],varargin{:});
            nfeval=nfeval+1;
            fworst=max([fworst fitness(1,1:mfit)]);
            Cf(1)=fcon_red(fworst,C);
            agentgoal(1,2)=Cf(1);
            agentgoal(1,1)=fitness(1,1);

            for k=2:npop
                [fitness(k),fitness(1:k-1)]=func(y(k,:),memories,y(1:k-1,:),fitness(1:k-1),varargin{:});
                C=cfunc(y(k,:),memories,y(1:k-1,:),fitness(1:k-1),varargin{:});
                nfeval=nfeval+1;
                fworst=max([fworst fitness(k)]);
                Cf(k)=fcon_red(fworst,C);
                agentgoal(k,2)=Cf(k);
                agentgoal(k,1)=fitness(k);
            end
            ...
            while filpop<npop&&nattempts<10
                ...
                for k=1:npop*2
                    ...
                    if yes&&filpop<npop
                        ...
                        if filpop==1
                            [fitness(filpop)]=func(y(filpop,:),memories,[],[],varargin{:});
                            C=cfunc(y(filpop,:),memories,[],[],varargin{:});
                        else
                            [fitness(filpop),fitness(1:filpop-1)]=func(y(filpop,:),memories,y(1:filpop-1,:),fitness(1:filpop-1),varargin{:});
                            C=cfunc(y(filpop,:),memories,y(1:filpop-1,:),fitness(1:filpop-1),varargin{:});
                        end
                        nfeval=nfeval+1;
                        fworst=max([fworst fitness(filpop)]);
                        Cf(filpop)=fcon_red(fworst,C);
                        agentgoal(filpop,2)=Cf(filpop);
                        agentgoal(filpop,1)=fitness(filpop);
            ...
            if filpop<npop
                ...
                for k=1:npop-filpop
                    ...
                    [fitness(1:filpop+k),fitness(1:filpop+k-1)]=func(y(filpop+k,:),memories,y(1:filpop+k-1,:),fitness(1:filpop+k-1),varargin{:});
                    C=cfunc(y(filpop+k,:),memories,y(1:filpop+k-1,:),fitness(1:filpop+k-1),varargin{:});
                    
                    nfeval=nfeval+1;
                    fworst=max([fworst fitness(filpop+k)]);
                    Cf(filpop+k)=fcon_red(fworst,C);
                    agentgoal(filpop+k,2)=Cf(filpop+k);
                    agentgoal(filpop+k,1)=fitness(filpop+k);
        ...
        %plot(memories(:,1),memories(:,2),'mo',y(:,1),y(:,2),'c*')

      
    end
%     hold on
%     if ~isempty(memories)
%     plot(y(:,1),fitness,'m*',memories(:,1),memories(:,2),'ro')
%     else
%     plot(y(:,1),fitness,'m*')        
%     end
%     drawnow
    ...
    elseif verbose==1
        if ~isempty(memories)
            [nfeval/ng maxd min(memories(:,lx+1)) min(fitness)]
        else
            [nfeval/ng maxd min(fitness) min(fitness)]
        end
