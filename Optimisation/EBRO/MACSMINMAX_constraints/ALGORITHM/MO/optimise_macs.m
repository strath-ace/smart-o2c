...
% Run MACS
if ~isfield(options, 'initial_memory') || isempty(options.initial_memory)
    [memory,nfeval,ener]=macs7v16OC(fitnessfcn,[],LB,UB,options,[],[],varargin{1:end});
else
    [memory,nfeval,ener]=macs7v16OC(fitnessfcn,options.initial_memory,LB,UB,options,[],[],varargin{1:end});
end
% Output
