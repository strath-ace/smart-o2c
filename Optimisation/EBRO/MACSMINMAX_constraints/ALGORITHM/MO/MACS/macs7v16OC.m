...
if isfield(options,'draw_flag')
    if ~isPositiveIntegerValuedNumeric(options.draw_flag+2) %0 is not counted as postive, this should do the trick
...
if ~isempty(memory)                                                          % if memory is already populated, no initialization is needed!
    ...
    switch mfit
        ...
        otherwise                                                           % Many objectives
            ...
            % n_lambda_aux is the (larger) set from which we choose
            
            lambda = rand(n_lambda*2,mfit);
            
            % normalisation of lambdas
            
            norms = sum(lambda.^2,2).^0.5;
            lambda = lambda./repmat(norms,1,3);
            
            % Uniformly spread selection of the randomly generated lambdas
            
            minl = min(lambda);
            maxl = max(lambda);
            
            [mm,ddt,eett,ene2t] = arch_shrk6([],[],[eye(mfit);lambda(1:n_lambda-mfit,:)],0,[],minl,maxl,0,mfit,n_lambda); % first pass, add extremas
            [mm,~,~,~] = arch_shrk6(mm,ddt,lambda(n_lambda+1-mfit:end,:),eett,ene2t,minl,maxl,0,mfit,n_lambda);
...
if ~isempty(memory_to_replace)
    memory = memory_to_replace;
end
...
while nfeval<options.maxnfeval
    ...
    if max(memory(:,end)<=0) && options.draw_flag>=0
        if mfit==3
