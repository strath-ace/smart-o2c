function [thetaVec,R,Phi,u_mat,DV,isLoopConverged,cont,TPrime1,Ucart,Vcart] = SphericalShaping(DepBody,ArrBody,t_dep,TOF,nr,toll,thetaStep,maxIter,a2,h,plotIter)
%25/09/15
%Function that implements the Spherical Shaping method (Novak, Vasile-2011)
% MODIFIED VERSION WITH STEP ACCELERATION
%------------------------------------------------------------------------%
%INPUT
...
% Transform the initial and arrival Kepler elements in spherical
% coordinates
...
% the Newton method)
% This coefficients are highly dependant upon the problem in consideration
% (for example, for Mars is good step = 1, for Neptune step = 10)
step = 10;
fixVal = 5.5;
ind = 0;
...
if isLoopConverged==1
    ...
    % Compute the control acceleration (in the t-n-h frame) 
    % with scalar products from the dynamics equations and the unit
    % vectors in the t-n-h frame (in AU/day^2)
    u_mat = [];
    uMod_vec = [];
